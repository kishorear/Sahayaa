...
${classification.aiNotes}`,
            ...
            resolvedAt: solutionResult.confidence > 0.7 ? /* @__PURE__ */ new Date() : void 0
          };
      ...
      buildTicketDescription(originalMessage, solutionSteps, instructions, similarTickets) {
...
`;
      ...
      mapUrgencyToTicketUrgency(agentUrgency) {
...
Respond with only the numbered steps, no additional explanation.`;
          ...
          if (steps.length === 0) {
            const sentences = text2.split(".").filter((s) => s.trim().length > 10);
    ...
    AgentService = class {
      ...
      async processWorkflow(request) {
        try {
          ...
          const { storage: storage3 } = await import("../../storage.js");
          const { classifyTicket: classifyTicket2 } = await import("../../ai.js");
          const startTime = Date.now();
          const title = this.extractTitleFromMessage(request.user_message);
          const classification = await classifyTicket2(title, request.user_message, request.tenant_id || 1);
...
${classification.aiNotes}`;
          ...
          const createdTicket = await storage3.createTicket(ticketData);
      ...
      extractTitleFromMessage(userMessage) {
        const words = userMessage.split(" ").filter((word) => word.length > 0);
      ...
      generateResolutionSteps(userMessage, classification) {
        const steps = [];
      ...
      mapComplexityToUrgency(complexity) {
        switch (complexity) {
          case "simple":
            return "low";
          case "complex":
...
function registerWidgetTicketRoutes(app2) {
  app2.post("/api/widget/create-ticket", async (req, res) => {
    try {
      ...
      const conversationForTitle = conversation.map((msg) => ({
        role: msg.role,
        content: msg.content
      }));
...
Your goal is to quickly gather issue details and create comprehensive support tickets rather than attempting complex fixes.`;
  ...
  app2.get("/api/metrics/summary", requireRole(["admin", "support-agent", "creator"]), async (req, res) => {
    try {
      ...
      const aiResolvedPercentage = totalInteractions > 0 ? Math.round(totalAiResolved / totalInteractions * 100) + "%" : "N/A";
      res.status(200).json({
        totalTickets,
        resolvedTickets,
        avgResponseTime,
