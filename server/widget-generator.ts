...
export async function generateWidgetPackage(config: WidgetConfig, res: Response): Promise<void> {
  ...
  // Include client integration package with authentication functionality
  const clientIntegrationDir = path.join(process.cwd(), 'public', 'downloads', 'widget', 'client-integration');
  
  if (fs.existsSync(clientIntegrationDir)) {
    // Read the auth widget with authentication capabilities
    const authWidgetJs = fs.readFileSync(path.join(clientIntegrationDir, 'auth-widget.js'), 'utf8');
    
    // Customize the auth widget with the provided configuration
    const customizedAuthWidgetJs = authWidgetJs
      .replace('__TENANT_ID__', config.tenantId.toString())
      .replace('__API_KEY__', config.apiKey)
      .replace('__PRIMARY_COLOR__', config.primaryColor)
      .replace('__POSITION__', config.position)
    ...
    // Add the client integration files to the archive
    archive.append(customizedAuthWidgetJs, { name: 'client-integration/auth-widget.js' });
    archive.file(path.join(clientIntegrationDir, 'auth-widget.css'), { name: 'client-integration/auth-widget.css' });
    archive.file(path.join(clientIntegrationDir, 'README.md'), { name: 'client-integration/README.md' });
    archive.file(path.join(clientIntegrationDir, 'INTEGRATION_GUIDE.md'), { name: 'client-integration/INTEGRATION_GUIDE.md' });
    archive.file(path.join(clientIntegrationDir, 'api-examples.js'), { name: 'client-integration/api-examples.js' });
    
    // Read the sample implementation template
    const clientSampleHtml = fs.readFileSync(path.join(clientIntegrationDir, 'sample-implementation.html'), 'utf8');
    
    // Customize the sample implementation with the provided configuration
    const customizedClientSampleHtml = clientSampleHtml
      .replace('__TENANT_ID__', config.tenantId.toString())
      .replace('__API_KEY__', config.apiKey)
      .replace('__PRIMARY_COLOR__', config.primaryColor)
      .replace('__POSITION__', config.position)
    ...
    archive.append(customizedClientSampleHtml, { name: 'client-integration/sample-implementation.html' });
    
    // Create client integration README with additional authentication instructions
    const clientReadmeContent = `# Support AI Chat Widget with Authentication

This directory contains an enhanced version of the Support AI Chat Widget that includes user authentication and dynamic AI provider configuration capabilities.

## Key Features

- User authentication (login and registration)
- Persistent login state across page navigations
- Dynamic AI provider configuration based on your Support AI settings
...
See the INTEGRATION_GUIDE.md file for step-by-step instructions on integrating the widget with authentication into your website.

For more detailed documentation, refer to the README.md file.

## Files Included

- auth-widget.js - Main widget script with authentication support
- auth-widget.css - CSS styling for the widget
- sample-implementation.html - Example implementation of the widget
- README.md - Detailed documentation
- INTEGRATION_GUIDE.md - Step-by-step integration instructions
- api-examples.js - Example API endpoint implementations

## Need Help?

Contact Support AI support for assistance with integration or customization.
`;
    
    archive.append(clientReadmeContent, { name: 'client-integration/README-FIRST.md' });
  }
